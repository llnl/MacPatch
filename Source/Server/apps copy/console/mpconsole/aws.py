	...
	def getS3Client(self):
		...
		if current_app.config['USE_AWS_S3']:
			try:
				...
				log_Error(f'Error creating S3 client: {str(e)}')
				raise
	...
	def fileExistsInS3(self, filePath):
		try:
			if not self.s3Client:
				log_Error("S3 client is not initialized")
				return False
				
			if not filePath:
				log_Error("File path is empty or None")
				return False
			...
			log_Error(f"File exists: {filePath}")
			return True
			
		except ClientError as e:
			error_code = e.response['error']['code'] if 'error' in e.response else e.response.get('Error', {}).get('Code', '')
			
			if error_code == '404':
				log_Error(f"File does not exist: {filePath}")
				return False
			else:
				# Other errors (permissions, bucket doesn't exist, etc.)
				log_Error(f"Error checking file existence for {filePath}: {str(e)}")
				return False
				
		except KeyError as e:
			log_Error(f"Missing configuration: {e}")
			return False
			
		except Exception as e:
			log_Error(f"Unexpected error checking file {filePath}: {str(e)}")
			return False

	def fileExistsInS3OLD(self, filePath):
		try:
