...
class ProvisionData(MPResource):
	...
	def get(self,client_id):
		try:
			...
			qGetSCFin = MpProvisionScript.query.filter(MpProvisionScript.active == 1, MpProvisionScript.scope == scope, MpProvisionScript.type == 2).order_by(MpProvisionScript.order.asc()).all()

			scriptsPre = []
			scriptsPost = []
			scriptsFinish = []
			tasks = []
			data = { "tasks": tasks, "scriptsPre": scriptsPre, "scriptsPost": scriptsPost, "scriptsFinish": scriptsFinish }
			...
			#print(qGetSCFin)
			if qGetSCFin is not None:
					for row in qGetSCFin:
							if row is not None:
									scriptsFinish.append(row.asDict)
					data['scriptsFinish'] = scriptsFinish
