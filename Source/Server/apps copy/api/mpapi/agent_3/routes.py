...
class UploadAgentPackages(MPResource):
	...
	def post(self, agent_id, token):
		...
		if not os.path.isdir(upload_dir):
			log_Info(f"Create directory {upload_dir} for agent files.")
			os.makedirs(upload_dir)
			serverCHOWN(upload_dir)

		for f in _files:
			log_Info(f"Saving file: {f.filename}")
			filename = secure_filename(f.filename)
			_pkg_file_path = os.path.join(upload_dir, filename)

			if os.path.exists(_pkg_file_path):
				log_Info(f"Removing existing agent file ({_pkg_file_path})")
				os.remove(_pkg_file_path)

			f.save(_pkg_file_path)
			serverCHOWN(_pkg_file_path)
			log_Info(f"File {_pkg_file_path} saved.")
