...
class SoftwarePackageDownload(MPResource):

	def __init__(self):
		self.reqparse = reqparse.RequestParser()
		super(SoftwarePackageDownload, self).__init__()
	...
	def get(self, suuid):
		...
		wsData = WSData()
		wsData.data = {}
		wsData.type = 'SoftwareDownload'
		wsResult.result = wsData


		_sw = MpSoftware.query.filter(MpSoftware.suuid == suuid).first()
		if _sw is not None:
			_result = f"/mp-content{_sw.sw_url}"
			if _sw.sw_useS3 == 1:
				_result = aws.getS3UrlForSoftware(_sw.sw_url)

				wsResult.errorno = 200
				wsResult.errormsg = ''
				wsData.data = {'data':_result}
				wsResult.data = wsData.toDict()
				return wsResult.resultWithSignature(), 200
			else:
				wsResult.errorno = 412
				wsResult.errormsg = 'Package is not setup for S3'
				wsData.data = {}
				wsResult.data = wsData.toDict()
				return wsResult.resultWithSignature(), 412
			
		else:
			wsResult.errorno = 404
			wsResult.errormsg = 'Package not found'
			wsData.data = {}
			wsResult.data = wsData.toDict()
			return wsResult.resultWithSignature(), 404
...
software_4_api.add_resource(SoftwarePackageDownload,			'/sw/download/<string:suuid>')