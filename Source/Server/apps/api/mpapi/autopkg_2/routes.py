...
class UploadAutoPKGPatch(MPResource):
	...
	def post(self, patch_id, token):
		...
		try:
			...
			if current_app.config['AWS_S3_BUCKET_AS_DEFAULT']:
				pkg_use_s3 = 1

				_fileData = savePatchFileToTMP(patch_id, file)
	...
	def savePatchFileToTMP(puuid, file):
		...
		if file is not None and len(file.filename) > 4:
			...
			result['fileHash'] = hashlib.md5(open(_file_path, 'rb').read(), usedforsecurity=False).hexdigest()
